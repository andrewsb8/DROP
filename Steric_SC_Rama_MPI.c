...
int main(int argc, char *argv[])
{
  ...
  if(myrank == 0)
  {
    ...
    writeXYZ(&prot, "trialanine_Ala2_SC.xyz", frame, 'm', 0, myrank);
    //FILE *free_spaces;
    for(int i = 1; i <= 180; i++)
    {
      double tmp = calculateDihedral(&prot, 5);
      rotateDihedral(&prot, 5, prot.dihedrals[5].dihedral_angle, 2, 0);
      printf("results: %d %f %f %f\n",i,calculateDihedral(&prot, 5), tmp, calculateDihedral(&prot, 5)-tmp);
      sprintf(frame, "%s %d", "Frame ", i);
      writeXYZ(&prot, "trialanine_Ala2_SC.xyz", frame, 'm', i, myrank);
      //if(checkClashes(&prot) == 0)
      //{
        //printf("%f\n", calculateDihedral(&prot, 5));
        //fprintf(free_spaces, "%f 1\n", calculateDihedral(&prot, 6));
      //}
