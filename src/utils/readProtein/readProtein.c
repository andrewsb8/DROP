	  ...
	  if (strcmp (substr (line, 0, 4), "ATOM") == 0)
		{
		  ...
		  if (isBackbone (prot->atoms[line_number].atom_type))
			{
			  prot->residues[res_num - 1].backbone_atoms[bb_atoms] =
				prot->atoms[line_number].atom_number;
			  bb_atoms += 1;
			}
		  else
			{
			  prot->residues[res_num - 1].sidechain_atoms[sc_atoms] =
				prot->atoms[line_number].atom_number;
			  sc_atoms += 1;
			}
	  ...
	  if (strcmp (line_split, "CONECT") == 0)
		{
		  ...
		  for (int k = 1; k < 3; k++)
			{
			  prot->bonds[line_number].bond_atomNumbers[k - 1] =
				atoi (stringT[k]);
			}
		  line_number++;
...
void
makeBondMatrix (struct protein *prot)
{
	  ...
	  if (prot->bonds[z].bond_atomNumbers[0] == atom1)	//atom is in first column of bonds list
		{
		  ...
		  found =
			recursivePairSearch (prot, atom1,
								 prot->bonds[z].bond_atomNumbers[1], atom2,
								 found, covalentBondCount);
	  ...
	  else if (prot->bonds[z].bond_atomNumbers[1] == atom1)	// atom is in second column of bond list
		{
		  ...
		  found =
			recursivePairSearch (prot, atom1,
								 prot->bonds[z].bond_atomNumbers[0], atom2,
								 found, covalentBondCount);
...
void
printCovalentBondMatrix (struct protein *prot, FILE * log)
{
...
*/
void
identifyDihedrals (struct protein *prot)
{
				  ...
				  if (strcmp
											 ...
											 1].atom_type) == 0)
					{
					  ...
					  for (int r = 0; r < prot->number_of_bonds; r++)
						{
						  if (prot->bonds[pairOne_index].
							  bond_atomNumbers[1] ==
							  prot->bonds[r].bond_atomNumbers[0]
							  && prot->bonds[pairTwo_index].
							  bond_atomNumbers[0] ==
							  prot->bonds[r].bond_atomNumbers[1])
							{
							  ...
							  *prot->dihedrals[prot->number_of_dihedrals].
								dihedral_angType = DihedralDefinitions[m][4];
							  ...
							  *prot->dihedrals[prot->number_of_dihedrals].
							  ...
							  if (prot->number_of_dihedrals > 0)
								{
								  prot->dihedrals =
									(struct _dihedrals *) realloc (prot->
																   dihedrals,
																   size *
																   (prot->
																	number_of_dihedrals
  ...
  for (int i = 0; i < prot->number_of_atoms; i++)
	{
	  ...
	  sprintf (line, "%s %f %f %f", prot->atoms[i].atom_type,
			   prot->atoms[i].coordinates[0], prot->atoms[i].coordinates[1],
			   prot->atoms[i].coordinates[2]);
...
void
writePDBmultiframe (struct protein *prot, FILE * fp, int frame)
{
  printf ("placeholder\n");
  return;
}
...
void
writePDBsingleframe (struct protein *prot, FILE * fp)
{
  fprintf (fp, "MODEL\t1\n");
  for (int i = 0; i < prot->number_of_atoms; i++)
	{
...
//https://github.com/gromacs/gromacs/blob/main/src/gromacs/fileio/pdbio.cpp
void
writePDBConect (struct protein *prot, FILE * fp)
{
