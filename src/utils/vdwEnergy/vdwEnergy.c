...
double
getParam (struct VDW_params *param, char *atom_name)
{
  switch (*atom_name)
	{
	case 'N':
...
double
mixedEpsilon (char *atom_one_name, char *atom_two_name)
{
...
double
mixedSigma (char *atom_one_name, char *atom_two_name)
{
...
double
pairVDWEnergy (double distance, double epsilon, double sigma)
{
...
double
calculateVDWEnergy (struct protein *prot, double gamma)
{
  double distance;
  double mixed_sigma;
		  ...
		  if (prot->atoms[i].covalent_bondArray[j - i - 1] > 3)
			{
			  ...
			  free (bond_vector);
			  if (distance <
				  gamma * getVDWRadii (&radii, prot->atoms[i].atom_name,
