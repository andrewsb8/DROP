...
double
calculateDihedral (struct protein *prot, int dihedralNumber)
{
  double angle;
  ...
  double *vec1 =
	vectorSubtract (prot->
					atoms[prot->dihedrals[dihedralNumber].
  ...
  double *vec3 =
	vectorSubtract (prot->
					atoms[prot->dihedrals[dihedralNumber].
						  dihedral_atomNumbers[2] - 1].coordinates,
					prot->atoms[prot->dihedrals[dihedralNumber].
								dihedral_atomNumbers[3] - 1].coordinates);
...
int
findDihedral (struct protein *prot, int rnum, char *dtype)
{
  int index = -1;
...
int *
findDihedrals (struct protein *prot, int rnum, FILE * log)
{
...
void
updatePositions (struct protein *prot, double newPositions[3], int atomNumber)
{
  for (int j = 0; j < 3; j++)
	{
				...
				double dihedralAngleChange, bool backbone)
{
  ...
  if (prot->
	  atoms[prot->dihedrals[dihedralNumber].dihedral_atomNumbers[2] -
			1].coordinates[2] < 0)
	{
  ...
  if (backbone)
	{
	  for (int i = atom_rotation_index - 1; i < prot->number_of_atoms; i++)
		{
		  double *tmp =
			vectorRotate (prot->atoms[i].coordinates, 2,
  ...
  else							//rotate side chain instead
	{
	  ...
	  for (int k = 0;
		   k <
		   prot->residues[prot->dihedrals[dihedralNumber].dihedral_resNum -
						  1].num_sc_atoms; k++)
		{
		  ...
		  if (found == 1)
			{
			  double *tmp =
				vectorRotate (prot->
							  atoms[prot->
									residues[prot->dihedrals[dihedralNumber].
											 dihedral_resNum -
											 1].sidechain_atoms[k] -
									1].coordinates, 2,
							  (M_PI / 180.0) * dihedralAngleChange);
