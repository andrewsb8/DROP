...
int
countClashes (struct protein *prot, FILE * log, bool list_clashes)
{
  double distance;
  int count = 0;
		  ...
		  if (prot->atoms[i].covalent_bondArray[j - i - 1] > 3)
			{
			  ...
			  double min_distance_allowed =
				getVDWRadii (&radii, prot->atoms[i].atom_name,
...
double
getVDWRadii (struct VDW *radii, char *atom_name, char *atom_name_2)
{
  switch (*atom_name)
	{
	case 'N':
